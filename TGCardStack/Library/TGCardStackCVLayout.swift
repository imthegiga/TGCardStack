...
public protocol TGCardStackDelegate {
...
/// Layout attributes for the TGCardStackCVLayout
open class TGCardStackCVLayoutAttributes: UICollectionViewLayoutAttributes {
    ...
    override open func copy(with zone: NSZone? = nil) -> Any {
        let attribute = super.copy(with: zone) as! TGCardStackCVLayoutAttributes
...
/// The TGCardStackCVLayout provides a card stack layout not quite similar like the apps Reminder and Wallet.
open class TGCardStackCVLayout: UICollectionViewLayout, UIGestureRecognizerDelegate {
    ...
    @IBInspectable open var collapseAllCards: Bool = false {
        didSet {
            self.flipRevealedCardBack(completion: {
                ...
                let collectionViewLayoutDelegate = self.collectionView?.delegate as? TGCardStackCVLayoutDelegate
    ...
    open func flipRevealedCard(toView: UIView, completion: (() -> Void)? = nil) {
        ...
        if let cardCell = self.revealedCardCell, self.revealedIndex >= 0 {
            ...
            UIView.transition(with: cardCell, duration: 0.8, options:[.transitionFlipFromRight], animations: { () -> Void in
    ...
    open func flipRevealedCardBack(completion: (() -> Void)? = nil) {
        ...
        if let cardCell = self.revealedCardCell {
            if let flipView = self.revealedCardFlipView {
                ...
                UIView.transition(with: cardCell, duration: 0.8, options:[.transitionFlipFromLeft], animations: { () -> Void in
    ...
    private var cardCollectionViewLayoutAttributes:[TGCardStackCVLayoutAttributes]!
    ...
    private var autoscrollDirection: TGCardStackScrollDirection = .unknown
    ...
    // MARK: Initialize TGCardStackCVLayout
    ...
    @objc internal func collectionViewTapGestureHandler() {
        ...
        if let tapLocation = self.collectionViewTapGestureRecognizer?.location(in: self.collectionView) {
            ...
            if let indexPath = self.collectionView?.indexPathForItem(at: tapLocation) {
                ...
                if cardStackDelegate != nil && (cardStackDelegate?.didFlipCard())! {
                    
                    cardStackDelegate?.flipBackCardIfAny()
                ...
                if indexPath.item == totalNumberOfCards - 1 {
                    ...
                    if (cardStackDelegate == nil || !cardStackDelegate!.areAllCellFilled().0) {
                        ...
                        cardStackDelegate?.animateFirstNonFilledCell()
                        
                    } else {
                        
                        cardStackDelegate?.performValidation()
                    }
    ...
    private func generateCardCollectionViewLayoutAttributes() -> [TGCardStackCVLayoutAttributes] {
        var cardCollectionViewLayoutAttributes: [TGCardStackCVLayoutAttributes] = []
        ...
        for itemIndex in startIndex..<endBeforeIndex {
            ...
            let cardLayoutAttribute = TGCardStackCVLayoutAttributes.init(forCellWith: indexPath)
    ...
    private func generateNonRevealedCardsAttribute(_ attribute: TGCardStackCVLayoutAttributes) {
    ...
    private func generateRevealedCardAttribute(_ attribute: TGCardStackCVLayoutAttributes) {
    ...
    private func generateBottomCardsAttribute(_ attribute: TGCardStackCVLayoutAttributes, bottomIndex:inout CGFloat) {
    ...
    enum TGCardStackScrollDirection : Int {
        case unknown = 0
        case up
        case down
    }
    
    private func setupScrollTimer(direction: TGCardStackScrollDirection) {
